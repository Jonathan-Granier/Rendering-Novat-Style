...
class Camera_TEST {
 public:
  ...
  Camera_TEST(float radius=1,const glm::vec3 &center=glm::vec3(0,0,0),int mode=PERSP);
...
inline glm::vec2 Camera_TEST::vec2ToGlm(const Vec2f &v) const {
  return glm::vec2(v.x(),v.y());
}

inline glm::vec3 Camera_TEST::vec3ToGlm(const Vec3f &v) const {
  return glm::vec3(v.x(),v.y(),v.z());
}

inline glm::vec4 Camera_TEST::vec4ToGlm(const Vec4f &v) const {
  return glm::vec4(v[0],v[1],v[2],v[3]);
}

inline glm::mat4 Camera_TEST::mat4ToGlm(const Mat4f &m) const {
  return glm::make_mat4(m.ptr());
}

inline Vec2f Camera_TEST::glmToVec2(const glm::vec2 &v) const {
  return Vec2f(v[0],v[1]);
}

inline Vec3f Camera_TEST::glmToVec3(const glm::vec3 &v) const {
  return Vec3f(v[0],v[1],v[2]);
}

inline Vec4f Camera_TEST::glmToVec4(const glm::vec4 &v) const {
  return Vec4f(v[0],v[1],v[2],v[3]);
}

inline Mat4f Camera_TEST::glmToMat4(const glm::mat4 &m) const {
...
inline void Camera_TEST::initRotation(const glm::vec2 &p) {
  _m = ROTATE;
  _p = glmToVec2(p);
  _t.beginTracking(_p);
}

inline void Camera_TEST::initMoveXY(const glm::vec2 &p) {
  _m = MOVEXY;
  _p = glmToVec2(p);
}

inline void Camera_TEST::initMoveZ(const glm::vec2 &p) {
  _m = MOVEZ;
  _p = glmToVec2(p);
}

inline void Camera_TEST::move(const glm::vec2 &p) {
  switch(_m) {
...
inline void Camera_TEST::rotate(const Vec2f &p) {
  Mat4f mo = _matm;
...
inline void Camera_TEST::moveXY(const Vec2f &p) {
...
inline void Camera_TEST::moveZ(const Vec2f &p) {
...
inline void Camera_TEST::updateCamVectors(const Mat4f &m) {
  _up    = Vec3f(m[0],m[4],m[8 ]);
  _right = Vec3f(m[1],m[5],m[9 ]);
  _view  = Vec3f(m[2],m[6],m[10]);
}

inline void Camera_TEST::updateCamDists(const Mat4f &m) {
