...
Camera_TEST::Camera_TEST(float radius,const glm::vec3 &center,int mode)
  : _m(NONE),
    _w(0),
    _h(0),
    _p(0,0),
...
void Camera_TEST::initialize(int w,int h,bool replace) {
  ...
  _w = w;
  _h = h;
  _t = TrackBall_TEST(rad*2,Vec2f(wo2,ho2));
  _f = 45.0f;
...
void Camera_TEST::setFovy(float f) {
  _f = f;
  initialize(_w,_h,false);
}

void Camera_TEST::setMode(int m) {
  if(_d!=m) {
    _d = m;
    initialize(_w,_h,false);
  }
